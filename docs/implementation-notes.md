...
- Rows show compact copied time in the top-right using social-style values like `now`, `1m`, `1h`, or `1d`; never-copied rows show `never`.
...
- Build 6 (Delivery UUID `29ab216c-44cb-44fd-b3a9-1594d2d3ff6a`) changed the never-copied row label from `new` to `never`; released to TestFlight with the same flow, note set to "Never-copied codes now show 'never' instead of 'new'.", state `WAITING_FOR_REVIEW`.
- The app publishes its three most recent entries as dynamic Home Screen quick actions. Selecting one generates and copies the current code, marks that entry as copied, and refreshes quick-action ordering.
- Build 7 (Delivery UUID `9731576d-13c0-4fab-a26c-e5097e32f400`) added Home Screen quick actions and kept newly added codes unarchived. It was added to the `External` beta group with the testing note "Copy your three most recent authenticator codes from Home Screen quick actions. Newly added codes now remain in the recent list." and submitted for external beta review (`WAITING_FOR_REVIEW`).
- SwiftUI owns the app's scene delegate, so warm Home Screen quick-action handling is installed on that existing delegate at runtime rather than replacing it; replacing or reconfiguring the delegate prevents SwiftUI from creating its window. Successful quick-action copies trigger the same copied toast as row taps after the app transition completes.
